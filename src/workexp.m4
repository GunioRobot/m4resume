...
define(`cv_note_ify',
	`ifdef(`do_rdfa',
	 	`ifelse(`$#', 0, ,
	 	        `$#', 1, <span property="cv:Notes" rdf:parseType="Literal">$1</span>,
	 	        ``<span property="cv:Notes" rdf:parseType="Literal">$1</span>',cv_note_ify(shift($@))')',
	$@)')

define(`RDFA_CISCO_1_ACHIEVEMENTS',
